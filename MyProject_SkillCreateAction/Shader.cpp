...
*//*************************/
void Shader::SetTexture(UINT slot, ID3D11ShaderResourceView* Resouce)
{
	m_pTextures[slot] = Resouce;
	switch (m_kind)
	{
	case Vertex:	GetContext()->VSSetShaderResources(slot, 1, &Resouce); break;
	case Pixel:		GetContext()->PSSetShaderResources(slot, 1, &Resouce); break;
	}
}
...
@brief  | コンストラクタ
*//*************************/
VertexShader::VertexShader(VSType In_eType)
...
@brief  | コンストラクタ
*//*************************/
PixelShader::PixelShader(PSType In_eType)
	: Shader(Shader::Pixel)
	, m_pPS(nullptr)
	, m_eType(In_eType)
{
